...
class LaporanController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['kehadiran-per-pegawai', 'kehadiran-pegawai', 'lihat-kehadiran', 'kehadiran-per-unit-kerja', 'kehadiran-pegawai-per-unit-kerja', 'tpp-per-pegawai', 'tpp-pegawai', 'detail-tpp-pegawai', 'tpp-per-unit-kerja', 'tpp-pegawai-per-unit-kerja', 'tpp-pajak-per-unit-kerja', 'tpp-pajak-pegawai-per-unit-kerja', 'tpp-tahunan-per-unit-kerja', 'tpp-pajak-tahunan-per-unit-kerja', 'print-kehadiran-pegawai-pdf', 'print-kehadiran-pegawai-per-unit-kerja-pdf', 'print-tpp-pegawai-pdf', 'print-tpp-pegawai-per-unit-kerja-pdf', 'print-tpp-pajak-pegawai-per-unit-kerja-pdf', 'print-tpp-pajak-tahunan-per-unit-kerja-pdf'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionTppTahunanPerUnitKerja()
    {
        $model = UnitKerja::find()->all();

        return $this->render('tpp-tahunan-per-unit-kerja', [
            'model'=>$model,
        ]);
    }
    ...
    public function actionTppPajakTahunanPerUnitKerja($unit)
    {
        if (Yii::$app->request->post()) {
            $year = Yii::$app->request->post('year');
        }
        ...
        $model = PrestasiPerilakuKerja::find()
            ->select([
                ...
                'tpp'=>'SUM(tpp_sebelum_pajak)',
                'pajak_tpp'=>'SUM(pajak_tpp)',
                'tpp_final'=>'SUM(tpp_final)'
                ])
            ->where(['unit_kerja'=>$unit])
            ->andWhere(['DATE_FORMAT(bulan, "%Y")'=>$year])
            ->groupBy('bulan')
            ->asArray()
            ->all();
        ...
        return $this->render('tpp-pajak-tahunan-per-unit-kerja', [
            'model'=>$model,
            'unit'=>$unit,
            'tahun'=>$year,
        ]);
    ...
    public function actionPrintTppPajakTahunanPerUnitKerjaPdf($unit, $year)
    {
        $model = PrestasiPerilakuKerja::find()
            ->select([
                'bulan'=>'DATE_FORMAT(bulan, "%m")',
        ...
        $content = $this->renderPartial('tpp-pajak-tahunan-per-unit-kerja-report', [
            'model'=>$model,
            'unit'=>$unit,
            'tahun'=>$year,
        ]);
        ...
        $pdf = new Pdf([
            ...
            'marginTop' => 45,
            // file name
            'filename' => 'REKAP-LAPORAN-TPP-'.$unit->kode.'-'.$year.'.pdf',
            // set mPDF properties on the fly
            'options' => ['title' => 'Report Title'],
            // call mPDF methods on the fly
            'methods' => [ 
