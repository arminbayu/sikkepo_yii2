...
class KehadiranController extends Controller
{
                        ...
                        try {
                            if (!$this->checkLibur($tgl_ymd) && $tgl->format('D') != 'Sat' && $tgl->format('D') != 'Sun') {
                                if (!$this->inputAbsenPegawai($kode_terminal, $no_absen, $nip, $tgl_ymd, $tgl->format('D'), $ym)) {
                                    ...
                                    // if (count($mnl = $this->checkAbsenManual($nip, $tgl_ymd)) >= 3) {
                                        ...
                                        // foreach ($mnl as $manl) {
                                        //     if ($manl->absen == 1)
                                        //         $masuk = new \DateTime($manl->jam, new \DateTimeZone(TIMEZONE));
                                        //     if ($manl->absen == 2)
                                        //         $siang = new \DateTime($manl->jam, new \DateTimeZone(TIMEZONE));
                                        //     if ($manl->absen == 3)
                                        //         $keluar = new \DateTime($manl->jam, new \DateTimeZone(TIMEZONE));
                                        // }
                                        
                                        // $manual_masuk = $this->checkAbsenManualPagi($nip, $tgl_ymd, 1);
                                        // $manual_siang = $this->checkAbsenManualSiang($nip, $tgl_ymd, 2);
                                        // $manual_keluar = $this->checkAbsenManualSore($nip, $tgl_ymd, 3);
    ...
    public function inputAbsenPegawai($kode_terminal, $no_absen, $nip, $tanggal, $hari, $ym) {
        ...
        foreach ($absen as $date => $data) {
            ...
            else if ($mobile = $this->checkAbsenMobile($nip, $tanggal)) {
                if (count($mobile) != 0) {
            ...
            //     $masuk z= new \DateTime($manual_masuk->jam, new \DateTimeZone(TIMEZONE));
            // }

            // // lihat data absen manual siang
            // if ($manual_siang = $this->checkAbsenManualSiang($nip, $tanggal, 2)) {
            //     $siang = new \DateTime($manual_siang->jam, new \DateTimeZone(TIMEZONE));
            // }

            // // lihat data absen manual sore
                    ...
                    try {
                        if (!$this->checkLibur($tgl_ymd) && $tgl->format('D') != 'Sat' && $tgl->format('D') != 'Sun') {
                            if (!$this->inputAbsenPegawai($kode_terminal, $no_absen, $nip, $tgl_ymd, $tgl->format('D'), $ym)) {
                                ...
                                else if (($mobile_masuk = $this->checkAbsenMobilePagi($nip, $tgl_ymd, 1)) && ($mobile_siang = $this->checkAbsenMobileSiang($nip, $tgl_ymd, 2)) && ($mobile_keluar = $this->checkAbsenMobileSore($nip, $tgl_ymd, 3))) {
                                    $max_jam_masuk = new \DateTime(MAX_MASUK, new \DateTimeZone(TIMEZONE));
    ...
    public function actionAbsManual() {
        $data = DataAbsenManual::find()->where(['NIP'=>'197909232011042001', 'tanggal'=>'2020-09-01'])->all();
    ...
    public function checkAbsenManualPagi($nip, $ymd, $absen) {
        ...
        $data = DataAbsenManual::findOne(['NIP'=>$nip, 'tanggal'=>$ymd, 'absen'=>$absen]);
        return $data;
    }
    ...
    public function checkAbsenMobilePagi($nip, $ymd, $absen) {
        //$data = DataAbsenManual::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->one();
        $data = DataAbsenMobile::findOne(['NIP'=>$nip, 'tanggal'=>$ymd, 'absen'=>$absen]);
        return $data;
    }
