...
class SiteController extends Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['logout', 'index', 'pengguna', 'tambah-pengguna', 'edit-pengguna', 'hapus-pengguna', 'terminal', 'tambah-terminal', 'edit-terminal', 'daftar-terminal', 'config', 'edit-config', 'ambil-data', 'data-absen', 'file-data-absen', 'no-absen', 'ka-unit', 'tambah-ka-unit', 'edit-ka-unit', 'hapus-ka-unit', 'cari-data-absen', 'pegawai', 'data-absen-pegawai', 'import-data'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionFileDataAbsen()
    {
        ...
        $bulan->sub(new \DateInterval('P3M'));
        $periode = $bulan->format('Y-m-d');

        $file = UploadData::find()
            ->where(['>', 'bulan', $periode])
            ->orderBy(['bulan'=>SORT_DESC])
            ->all();
