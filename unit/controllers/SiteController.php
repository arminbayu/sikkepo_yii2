...
class SiteController extends Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['logout', 'index', 'pengguna', 'reset-password', 'tambah-pengguna', 'edit-pengguna', 'hapus-pengguna', 'terminal', 'tambah-terminal', 'edit-terminal', 'daftar-terminal', 'export-data-absen', 'export-data', 'upload-data', 'upload-data-absen', 'ambil-data', 'data-absen', 'no-absen', 'cari-data-absen', 'pegawai', 'unit-kerja', 'edit-unit-kerja', 'edit-koordinat', 'update-koordinat', 'data-absen-pegawai'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function checkFileAbsen($terminal, $ym) {
        $data = UploadData::find()->where(['terminal'=>$terminal, 'file'=>$ym.'-'.$terminal.'.json'])->exists();
        return $data;
    }
    ...
    public function actionPegawai($nip)
    {
    ...
    public function actionEditKoordinat($id)
    {
        $model = $this->findModelUnitKerja($id);

        return $this->render('koordinat', [
            'model' => $model,
            'id' => $id,
        ]);
    }
    ...
    public function actionUpdateKoordinat()
    {
        $id = Yii::$app->request->post('unit');
        $latLng = Yii::$app->request->post('koordinat');

        $model = $this->findModelUnitKerja($id);
        $model->koordinat = $latLng;
    ...
    {
        if (($model = StaffUser::findOne($id)) !== null) {
            return $model;
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
