...
use common\models\DataAbsenMobile;
...
class KehadiranController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['upacara', 'upacara-pegawai', 'edit-upacara', 'edit-upacara-pegawai', 'ketidakhadiran', 'input-ketidakhadiran', 'edit-ketidakhadiran', 'edit-ketidakhadiran-pegawai', 'delete-ketidakhadiran', 'tugas-luar', 'input-tugas-luar', 'daftar-tugas-luar', 'lihat-photo-tugas-luar', 'edit-tugas-luar', 'edit-tugas-luar-pegawai', 'delete-tugas-luar', 'cancel-tugas-luar', 'absen-manual', 'absen-manual-pegawai', 'edit-absen-manual', 'edit-absen-manual-pegawai', 'delete-absen-manual', 'jadwal-wfh', 'simpan-jadwal', 'jadwal-wfh-pegawai', 'edit-jadwal-wfh', 'edit-jadwal-wfh-pegawai', 'delete-jadwal-wfh', 'kehadiran-per-unit-kerja', 'proses-kehadiran-per-unit-kerja', 'kehadiran-pegawai-per-unit-kerja', 'lihat-kehadiran'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionInputTugasLuar()
    {
        ...
        $no_surat = Yii::$app->request->post('no_surat');
    ...
    public function actionDaftarTugasLuar()
    {
        ...
        return $this->render('daftar-tugas-luar', [
                'model' => $model,
            ]);
    }
    ...
    public function actionLihatPhotoTugasLuar($id)
    {
        $model = TugasLuar::find()
            ->where(['id'=>$id])
            ->one();

        return $this->renderAjax('lihat-photo-tugas-luar', [
                'model' => $model,
            ]);
    }
    ...
    public function actionCancelTugasLuar($id)
    {
        $model = $this->findModelTugasLuar($id);

        $model->status = 0;

        if ($model->save()) {
            Yii::$app->session->setFlash('success', 'Status Tugas Luar berhasil diubah!');
        }
        else {
            Yii::$app->session->setFlash('failed', 'Status Tugas Luar gagal diubah!');
        }
        
        return $this->redirect(['daftar-tugas-luar']);
    }
    ...
    public function actionJadwalWfhPegawai($dari, $sampai)
    {
        $model = DataPegawai::find()
          ->where(['unit_kerja'=>$this->unit, 'status'=>1])
          ->all();
    ...
    public function actionProsesKehadiranPerUnitKerja()
    {
        ...
        foreach ($model as $data) {
            $this->actionProsesKehadiranPlusMobile($data->NIP, $data->kode_terminal, $data->no_absen, $ym);
        }
                    ...
                    try {
                        if (!$this->checkLibur($tgl_ymd) && $tgl->format('D') != 'Sat' && $tgl->format('D') != 'Sun') {
                            if (!$this->inputAbsenPegawai($kode_terminal, $no_absen, $nip, $tgl_ymd, $tgl->format('D'), $ym)) {
                                ...
                                // begin modified 05/03/2019

                                if (($manual_masuk = $this->checkAbsenManualPagi($nip, $tgl_ymd, 1)) && ($manual_siang = $this->checkAbsenManualSiang($nip, $tgl_ymd, 2)) && ($manual_keluar = $this->checkAbsenManualSore($nip, $tgl_ymd, 3))) {
                                // if ($this->checkAbsenManualPagi($nip, $tgl->format('Y-m-d'), 1) && $this->checkAbsenManualSiang($nip, $tgl->format('Y-m-d'), 2) && $this->checkAbsenManualSore($nip, $tgl->format('Y-m-d'), 3)) {
                                    ...
                                    // $manual_masuk = $this->checkAbsenManualPagi($nip, $tgl->format('Y-m-d'), 1);
                                    // $manual_siang = $this->checkAbsenManualSiang($nip, $tgl->format('Y-m-d'), 2);
                                    // $manual_keluar = $this->checkAbsenManualSore($nip, $tgl->format('Y-m-d'), 3);
                                    
                                    $masuk = new \DateTime($manual_masuk->jam, new \DateTimeZone(TIMEZONE));
                                    $siang = new \DateTime($manual_siang->jam, new \DateTimeZone(TIMEZONE));
                                    $keluar = new \DateTime($manual_keluar->jam, new \DateTimeZone(TIMEZONE));
    ...
    public function inputAbsenPegawai($kode_terminal, $no_absen, $nip, $tanggal, $hari, $ym) {
        ...
        foreach ($absen as $date => $data) {
            ...
            // lihat data absen mobile
            if ($mobile = $this->checkAbsenMobile($nip, $tanggal)) {
                if (count($mobile) != 0) {
                    foreach ($mobile as $mb) {
                        if ($mb->absen == 1)
                            $masuk = new \DateTime($mb->jam, new \DateTimeZone(TIMEZONE));
                        elseif ($mb->absen == 2)
                            $siang = new \DateTime($mb->jam, new \DateTimeZone(TIMEZONE));
                        else
                            $keluar = new \DateTime($mb->jam, new \DateTimeZone(TIMEZONE));
                    }
                }
            }
            // lihat data absen manual
            else if ($manual = $this->checkAbsenManual($nip, $tanggal)) {
                if (count($manual) != 0) {
            ...
            // // lihat data absen manual pagi
            // if ($manual_masuk = $this->checkAbsenManualPagi($nip, $tanggal, 1)) {
            //     $masuk = new \DateTime($manual_masuk->jam, new \DateTimeZone(TIMEZONE));
            // }

            // // lihat data absen manual pagi
            // if ($manual_siang = $this->checkAbsenManualSiang($nip, $tanggal, 2)) {
            //     $siang = new \DateTime($manual_siang->jam, new \DateTimeZone(TIMEZONE));
            // }

            // // lihat data absen manual pagi
            // if ($manual_keluar = $this->checkAbsenManualSore($nip, $tanggal, 3)) {
            //     $keluar = new \DateTime($manual_keluar->jam, new \DateTimeZone(TIMEZONE));
            // }
    ...
    public function actionProsesKehadiranPlusMobile($nip, $kode_terminal, $no_absen, $ym)
    {
        $ym_ = \DateTime::createFromFormat('Y-m', $ym);
        $month = $ym_->format('m');
        $year = $ym_->format('Y');
                    ...
                    try {
                        if (!$this->checkLibur($tgl_ymd) && $tgl->format('D') != 'Sat' && $tgl->format('D') != 'Sun') {
                            if (!$this->inputAbsenPegawai($kode_terminal, $no_absen, $nip, $tgl_ymd, $tgl->format('D'), $ym)) {
                                ...
                                if (($mobile_masuk = $this->checkAbsenMobilePagi($nip, $tgl_ymd, 1)) && ($mobile_siang = $this->checkAbsenMobileSiang($nip, $tgl_ymd, 2)) && ($mobile_keluar = $this->checkAbsenMobileSore($nip, $tgl_ymd, 3))) {
                                    $max_jam_masuk = new \DateTime(MAX_MASUK, new \DateTimeZone(TIMEZONE));
                                    ...
                                    $masuk = new \DateTime($mobile_masuk->jam, new \DateTimeZone(TIMEZONE));
                                    $siang = new \DateTime($mobile_siang->jam, new \DateTimeZone(TIMEZONE));
                                    $keluar = new \DateTime($mobile_keluar->jam, new \DateTimeZone(TIMEZONE));
                                ...
                                else if (($manual_masuk = $this->checkAbsenManualPagi($nip, $tgl_ymd, 1)) && ($manual_siang = $this->checkAbsenManualSiang($nip, $tgl_ymd, 2)) && ($manual_keluar = $this->checkAbsenManualSore($nip, $tgl_ymd, 3))) {
                                    $max_jam_masuk = new \DateTime(MAX_MASUK, new \DateTimeZone(TIMEZONE));
    ...
    public function checkDataAbsenManual($nip, $ymd, $absen) {
        $data = DataAbsenManual::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->exists();
        return $data;
    }

    public function checkAbsenManual($nip, $ymd) {
        $data = DataAbsenManual::find()->where(['NIP'=>$nip, 'tanggal'=>$ymd])->all();
        return $data;
    }

    public function checkAbsenManualPagi($nip, $ymd, $absen) {
        $data = DataAbsenManual::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->one();
        return $data;
    }

    public function checkAbsenManualSiang($nip, $ymd, $absen) {
        $data = DataAbsenManual::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->one();
        return $data;
    }

    public function checkAbsenManualSore($nip, $ymd, $absen) {
        $data = DataAbsenManual::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->one();
        return $data;
    }

    public function checkAbsenMobile($nip, $ymd) {
        $data = DataAbsenMobile::find()->where(['NIP'=>$nip, 'tanggal'=>$ymd])->all();
        return $data;
    }

    public function checkAbsenMobilePagi($nip, $ymd, $absen) {
        $data = DataAbsenMobile::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->one();
        return $data;
    }

    public function checkAbsenMobileSiang($nip, $ymd, $absen) {
        $data = DataAbsenMobile::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->one();
        return $data;
    }

    public function checkAbsenMobileSore($nip, $ymd, $absen) {
        $data = DataAbsenMobile::find()->where(['NIP'=>$nip, 'DATE_FORMAT(tanggal, "%Y-%m-%d")'=>$ymd, 'absen'=>$absen])->one();
        return $data;
    }
    ...
    {
        if (($model = DataAbsenManual::findOne($id)) !== null) {
            return $model;
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
