...
class TppController extends Controller
{
    ...
    public function actionProsesTppPerUnitKerja()
    {
        $bulan = new \DateTime('first day of this month', new \DateTimeZone(TIMEZONE));
        $bulan->sub(new \DateInterval('P1M'));
        $ym = $bulan->format('Y-m');
        ...
        $model = AbsenPegawai::find()
            ->where(['unit_kerja'=>$this->unit, 'DATE_FORMAT(tanggal, "%Y-%m")'=>$ym])
            ->innerJoinWith(['nip'])
            ->groupBy(['NIP'])
            ->all();
    ...
    public function actionSavePPK($nip, $ym)
    {
        ...
        try {
            if (!$this->checkPPKPegawai($nip, $ym)) {
                ...
                $tpp_sebelum_pajak = ($ppk->nip->kodeTpp) ? round((($bobot_ketepatan + $bobot_kehadiran + $bobot_kinerja)/100) * $ppk->nip->kodeTpp->tpp) : '';
    ...
    public function actionTppPegawaiPerUnitKerja()
    {
        $bulan = new \DateTime('first day of this month', new \DateTimeZone(TIMEZONE));
        $bulan->sub(new \DateInterval('P1M'));
        $ym = $bulan->format('Y-m');
        ...
        if (Model::loadMultiple($model, Yii::$app->request->post()) && Model::validateMultiple($model)) {
            ...
            return $this->redirect(['tpp-pegawai-per-unit-kerja']);
        }
