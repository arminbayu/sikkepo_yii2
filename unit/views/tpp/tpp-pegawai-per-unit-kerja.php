        ...
        <div class="col-md-12">
            ...
            <table class="table table-striped">
                ...
                <?php
                    ...
                    $data_bobot_kehadiran = ($data->hadir == 0 && $data->bobot_ketepatan == 0 ) ? 0 : $data->bobot_kehadiran;
...
$this->registerJs($script[$index]);
                ...
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $data->nip->nama ?></td>
                    <?= $form->field($data, "[$index]unit_kerja")->hiddenInput(['style' => 'width:35px; height:10px; margin-bottom:-10px; text-align:center; border:0; background-color:transparent', 'value' => $unit->kode])->label(false) ?>
                    <td style="text-align: center;"><?= $data->hadir ?></td>
                    <td style="text-align: center;"><?= $form->field($data, "[$index]bobot_kehadiran")->textInput(['style' => 'width:50px; height:10px; margin-bottom:-10px; text-align:center; border:0; background-color:transparent', 'readonly' => true, 'value' => ($data->hadir == 0 && $data->bobot_ketepatan == 0 ) ? 0 : $data->bobot_kehadiran])->label(false) ?></td>
                    <td style="text-align: center;"><?= $data->bobot_ketepatan ?></td>
                    <td style="text-align: center;"><?= $form->field($data, "[$index]kinerja")->textInput(['style' => 'width:50px; height:10px; margin-bottom:-10px; text-align:center', 'readonly' => true])->label(false) ?></td>
                    ...
                    <td style="text-align: right;"><?= ($data_tpp) ? Yii::$app->formatter->asDecimal($data_tpp) : NULL ?></td>
                    <?= $form->field($data, "[$index]tpp")->hiddenInput(['style' => 'width:80px; height:10px; margin-bottom:-10px; text-align:center; border:0; background-color:transparent', 'readonly' => true, 'value' => ($data_tpp) ? $data_tpp : NULL])->label(false) ?>

                    <td style="text-align: center;"><div id="tpp-sebelum-pajak-<?= $index ?>"><?= Yii::$app->formatter->asDecimal($data->tpp_sebelum_pajak) ?></div></td>
                    ...
                    <td style="text-align: center;"><div id="pajak-tpp-<?= $index ?>"><?= Yii::$app->formatter->asDecimal($data->pajak_tpp) ?></div></td>
                    ...
                    <td style="text-align: right;"><div id="tpp-final-<?= $index ?>"><?= Yii::$app->formatter->asDecimal($data->tpp_final) ?></div></td>
