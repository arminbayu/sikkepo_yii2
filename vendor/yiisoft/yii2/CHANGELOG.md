...
- Bug #6991: Fixed exception when using `{use class='yii\bootstrap5\Nav' type='function'}` (ivanlemeshev)
...
- Bug #6672: `yii\bootstrap5\Dropdown` should register client event handlers (qiangxue)
...
- Bug #5570: `yii\bootstrap5\Tabs` would throw an exception if `content` is not set for one of its `items` (RomeroMsk)
...
- Bug #6150: `yii\bootstrap5\Tabs` dropdown IDs were generated incorrectly (samdark)
...
- Enh #4146: Added `yii\bootstrap5\ButtonDropdown::$containerOptions` (samdark)
- Enh #4181: Added `yii\bootstrap5\Modal::$headerOptions` and `yii\bootstrap5\Modal::$footerOptions` (tuxoff, samdark)
...
- Enh #4450: Added `yii\bootstrap5\Nav::renderDropdown()` (qiangxue)
...
- Enh #5494: Added support for specifying a menu header as a configuration array in `yii\bootstrap5\Dropdown` (hiltonjanfield, qiangxue)
...
- Enh #5735: Added `yii\bootstrap5\Tabs::renderTabContent` to support manually rendering tab contents (RomeroMsk)
- Enh #5770: Added more PHP error names for `ErrorException` (mongosoft)
- Enh #5799: `yii\bootstrap5\ButtonGroup::buttons` can take all options that are supported by `yii\bootstrap5\Button` (aleksanderd)
...
- Bug #5323: Nested dropdown does not work for `yii\bootstrap5\DropDown` (aryraditya)
- Bug #5336: `yii\bootstrap5\DropDown` should register bootstrap plugin asset (zelenin)
...
- Bug #3443: Fixed `yii\bootstrap5\Nav` and `yii\bootstrap5\Dropdown` were generating wrong ids for submenus (arturf)
...
- Enh #1562: Added `yii\bootstrap5\Tabs::linkOptions` (kartik-v)
...
- Enh #1881: Improved `yii\bootstrap5\NavBar` with `containerOptions`, `innerContainerOptions` and `renderInnerContainer` (creocoder)
...
- New #3029: Added `yii\bootstrap5\ActiveForm` and `yii\bootstrap5\ActiveField` (mikehaertl)
