...
class SiteController extends Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['logout', 'index', 'pengguna', 'tambah-pengguna', 'edit-pengguna', 'hapus-pengguna', 'terminal', 'tambah-terminal', 'edit-terminal', 'daftar-terminal', 'config', 'edit-config', 'ambil-data', 'data-absen', 'file-data-absen', 'no-absen', 'ka-unit', 'tambah-ka-unit', 'edit-ka-unit', 'hapus-ka-unit', 'cari-data-absen', 'pegawai', 'data-absen-pegawai', 'import-data', 'unit-kerja', 'data-pegawai-per-unit-kerja', 'data-absen-mobile'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    {
        $model = $this->findModelConfig($id);

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['config']);
        } else {
            return $this->render('edit-config', [
                'model' => $model,
                'id' => $id,
            ]);
        }
    ...
    public function actionDataAbsenMobile($nip)
    {
        if (Yii::$app->request->post()) {
            $month = Yii::$app->request->post('month');
            $year = Yii::$app->request->post('year');
        ...
        return $this->render('data-absen-mobile', [
            'model'=>$model,
            'bulan'=>$ym,
        ]);
    }
    ...
    public function actionImportData($terminal, $ym)
    {
        ...
        if (empty($requests)) {
            ...
            UploadData::updateAll(['status' => 1], ['=', 'file', $ym.'-'.$terminal.'.json']);
        }
    ...
    protected function findModelConfig($id)
    {
